...
(`pc`/`ct`/`imp` params — `src/app/urlState.ts`, restored via the store's
`replaceAll` action), rendered by `FilterBar` as chip groups in Hebrew
(קטגוריית אישים · סוג תוכן · חשיבות).

## Combination semantics

**OR within a dimension, AND across dimensions.** Selecting "מנהיגים" and
"סופרים" shows people in either category; adding a minimum importance of 70 then
also drops everything below that floor.

```ts
function passes(item: TimelineItem, f: FilterState): boolean {
  return matchesCategory(item, f.personCategoryIds) // OR within
